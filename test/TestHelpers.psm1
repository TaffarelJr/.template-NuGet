...
.PARAMETER TestRun
A custom object containing test run configuration details,
including the project, framework, and directory information.
#>
...
<#
.SYNOPSIS
Uploads a test coverage report to Codecov.

.DESCRIPTION
Finds the Cobertura coverage report within the given test run's output directory
and uploads it to Codecov using the `codecov` CLI.
...
function Publish-TestCoverage {
    ...
    process {
        ...
        if (-not $file) {
            throw "Test coverage not found: '$($TestRun.Output)'"
        }
...
<#
.SYNOPSIS
Uploads a test results report to Codecov.

.DESCRIPTION
Finds the JUnit test results report within the given test run's output directory
and uploads it to Codecov using the `codecov` CLI.
...
function Publish-TestResults {
    ...
    process {
        ...
        if (-not $file) {
            throw "Test results not found: '$($TestRun.Output)'"
        }
